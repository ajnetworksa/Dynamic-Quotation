...
import { Plus, Trash2, Save, Printer, RefreshCw, Download, FileSpreadsheet, Send, Loader2, ArrowUp, ArrowDown, Copy, Bookmark, BookOpen, Languages, ChevronDown, Search, Bot, GripVertical, AlertTriangle, Users, FileText, StickyNote, X, Lock, Unlock, CheckCircle, Clock } from 'lucide-react';
...
export default function QuoteForm() {
  ...
  // Helper to read cached settings or empty defaults so terms always derive from settings
  const getCachedTerms = (): Record<string, any> => {
    try {
      const cached = localStorage.getItem('cached_documentTerms');
      if (cached) return JSON.parse(cached);
    } catch {}
    return {};
  };

  const initialTerms = getCachedTerms();
  const defaultTermsRef = useRef<Record<string, any>>(initialTerms);

  const [noteHeader, setNoteHeader] = useState(initialTerms.noteHeader || 'NOTE:');

  // Main notice / note that appears at the top of the Terms section.
  const [note, setNote] = useState(initialTerms.note || '');
  const [noteAr, setNoteAr] = useState(initialTerms.noteAr || '');
  const [lastNoteTrigger, setLastNoteTrigger] = useState(initialTerms.note || '');

  // Payment terms shown on the quote.
  const [payment, setPayment] = useState(initialTerms.payment || '');
  const [paymentAr, setPaymentAr] = useState(initialTerms.paymentAr || '');

  // Warranty terms.
  const [warranty, setWarranty] = useState(initialTerms.warranty || '');
  const [warrantyAr, setWarrantyAr] = useState(initialTerms.warrantyAr || '');

  // Number of workers / technicians for the project.
  const [manpower, setManpower] = useState(initialTerms.manpower || '');
  const [manpowerAr, setManpowerAr] = useState(initialTerms.manpowerAr || '');

  // How long before team can mobilize / start the work.
  const [mobilization, setMobilization] = useState(initialTerms.mobilization || '');
  const [mobilizationAr, setMobilizationAr] = useState(initialTerms.mobilizationAr || '');

  // How long the work itself will take.
  const [duration, setDuration] = useState(initialTerms.duration || '');
  const [durationAr, setDurationAr] = useState(initialTerms.durationAr || '');

  // Bank/payment details.
  const [bankDetails, setBankDetails] = useState(initialTerms.bankDetails || '');
  const [bankDetailsAr, setBankDetailsAr] = useState(initialTerms.bankDetailsAr || '');

  // Closing footer message (shown in text if no footer image is uploaded).
  const [footer, setFooter] = useState(initialTerms.footer || '');
  const [footerAr, setFooterAr] = useState(initialTerms.footerAr || '');

  const applyDefaultTerms = (dt: Record<string, any>) => {
    if (!dt) return;
    setNoteHeader(dt.noteHeader || 'NOTE:');
    setNote(dt.note || '');
    setLastNoteTrigger(dt.note || '');
    setNoteAr(dt.noteAr || '');
    setPayment(dt.payment || '');
    setPaymentAr(dt.paymentAr || '');
    setWarranty(dt.warranty || '');
    setWarrantyAr(dt.warrantyAr || '');
    setManpower(dt.manpower || '');
    setManpowerAr(dt.manpowerAr || '');
    setMobilization(dt.mobilization || '');
    setMobilizationAr(dt.mobilizationAr || '');
    setDuration(dt.duration || '');
    setDurationAr(dt.durationAr || '');
    setBankDetails(dt.bankDetails || '');
    setBankDetailsAr(dt.bankDetailsAr || '');
    setFooter(dt.footer || '');
    setFooterAr(dt.footerAr || '');

    // Reset visibility toggles according to configured defaults
    setShowNote(dt.showNote !== undefined ? !!dt.showNote : true);
    setShowPayment(dt.showPayment !== undefined ? !!dt.showPayment : true);
    setShowWarranty(dt.showWarranty !== undefined ? !!dt.showWarranty : true);
    setShowManpower(dt.showManpower !== undefined ? !!dt.showManpower : true);
    setShowMobilization(dt.showMobilization !== undefined ? !!dt.showMobilization : true);
    setShowDuration(dt.showDuration !== undefined ? !!dt.showDuration : true);
    setShowBankDetails(dt.showBankDetails !== undefined ? !!dt.showBankDetails : true);
    setShowCustomField(dt.showCustomField !== undefined ? !!dt.showCustomField : false);
  };
  ...
  // Visibility Toggles - initialized from saved settings template
  const [showNote, setShowNote] = useState(initialTerms.showNote !== undefined ? !!initialTerms.showNote : true);
  const [showPayment, setShowPayment] = useState(initialTerms.showPayment !== undefined ? !!initialTerms.showPayment : true);
  const [showWarranty, setShowWarranty] = useState(initialTerms.showWarranty !== undefined ? !!initialTerms.showWarranty : true);
  const [showManpower, setShowManpower] = useState(initialTerms.showManpower !== undefined ? !!initialTerms.showManpower : true);
  const [showMobilization, setShowMobilization] = useState(initialTerms.showMobilization !== undefined ? !!initialTerms.showMobilization : true);
  const [showDuration, setShowDuration] = useState(initialTerms.showDuration !== undefined ? !!initialTerms.showDuration : true);
  const [showBankDetails, setShowBankDetails] = useState(initialTerms.showBankDetails !== undefined ? !!initialTerms.showBankDetails : true);
  const [showCustomField, setShowCustomField] = useState(initialTerms.showCustomField !== undefined ? !!initialTerms.showCustomField : false);
  ...
  }, [bankDetails, bankDetailsAr, note, noteAr, customFields, showBankDetails, showNote]);


  const fetchDocumentTerms = useCallback(async (applyImmediately = false) => {
    try {
      const termsRes = await fetch('/api/settings/documentTerms', {
        headers: getAuthHeaders()
      });
      if (termsRes.ok) {
        const termsData = await termsRes.json();
        if (termsData?.value) {
          const parsed = JSON.parse(termsData.value);
          defaultTermsRef.current = parsed;
          try {
            localStorage.setItem('cached_documentTerms', JSON.stringify(parsed));
          } catch {}
          if (applyImmediately) {
            applyDefaultTerms(parsed);
          }
          return parsed;
        }
      }
    } catch (e) {
      console.error('Failed to load documentTerms settings:', e);
    }
    const cached = getCachedTerms();
    if (applyImmediately && cached && Object.keys(cached).length > 0) {
      applyDefaultTerms(cached);
    }
    return cached;
  }, []);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      loadTemplates();
      // Load configured default terms from settings
      await fetchDocumentTerms(!recallQuoteId && !quoteId && !localStorage.getItem(DRAFT_KEY));

      try {
        const tmRes = await fetch('/api/settings/translationMode', { headers: getAuthHeaders() });
        if (tmRes.ok) {
          const tmData = await tmRes.json();
          if (tmData.value) {
            setTranslationMode(tmData.value as 'manual' | 'auto');
            localStorage.setItem('translationMode', tmData.value);
          }
        }
      } catch (e) {
  ...
  // Listen for live updates when documentTerms are saved in Settings tab
  useEffect(() => {
    const handleTermsUpdated = (e: any) => {
      const terms = e.detail;
      if (terms) {
        defaultTermsRef.current = terms;
        try {
          localStorage.setItem('cached_documentTerms', JSON.stringify(terms));
        } catch {}
        // If current quote is a blank fresh quote, apply immediately
        const isBlank = !recallQuoteId && !selectedCustomerId && !subject && items.every(i => !i.description.trim());
        if (isBlank) {
          applyDefaultTerms(terms);
        }
      }
    };
  ...
  useEffect(() => {
    if (location.pathname === '/quote' && document.visibilityState === 'visible') {
      ...
      fetchProducts();
      fetchSuppliers();
      fetchDocumentTerms(!recallQuoteId && !selectedCustomerId && !subject && items.every(i => !i.description.trim()));
    }
  }, [location.pathname, recallQuoteId, selectedCustomerId, subject, items, fetchDocumentTerms]);
  ...
  const handleTranslateAll = async () => {
    ...
    for (let i = 0; i < newItems.length; i++) {
      if (newItems[i].description) {
        const prod = newItems[i].product_id ? products.find(p => p.id === newItems[i].product_id) : null;
        if (prod?.description_ar) {
          newItems[i].description_ar = prod.description_ar;
          itemsChanged = true;
        } else {
  ...
  // Auto-translate Debounced Effect for Notes (Only in Auto mode)
  useEffect(() => {
    if (translationMode !== 'auto') return;
  ...
  }, [note, lastNoteTrigger, translationMode]);
  ...
  const fetchQuote = async (id: string, customersList: Customer[] = customers, productsList: Product[] = products) => {
    ...
    if (res.ok) {
      ...
      const pick = (draftVal: any, dbVal: any, defaultVal: any = '') => {
      ...
      setDate(pick(parsedDraft?.date, data.date, new Date().toISOString().split('T')[0]));
      setExpiryDate(pick(parsedDraft?.expiryDate, data.expiry_date, ''));
      const custId = pick(parsedDraft?.selectedCustomerId, data.customer_id, '');
      ...
      setSubject(pick(parsedDraft?.subject, data.subject, ''));
      setSubjectAr(pick(parsedDraft?.subjectAr, data.subject_ar, ''));
      const dt = defaultTermsRef.current;
      setNoteHeader(pick(parsedDraft?.noteHeader, data.note_header, dt.noteHeader || 'NOTE:'));
      setNote(pick(parsedDraft?.note, data.note, dt.note || ''));
      setNoteAr(pick(parsedDraft?.noteAr, data.note_ar, dt.noteAr || ''));
      const loadedMode = pick(parsedDraft?.discount_type || parsedDraft?.discountMode, data.discount_type, 'amount');
      const loadedRate = parsedDraft?.discount_rate !== undefined ? parsedDraft.discount_rate : (parsedDraft?.discountRate !== undefined ? parsedDraft.discountRate : (data.discount_rate ?? 0));
      ...
      setPayment(pick(parsedDraft?.payment, data.payment, dt.payment || ''));
      setPaymentAr(pick(parsedDraft?.paymentAr, data.payment_ar, dt.paymentAr || ''));
      setWarranty(pick(parsedDraft?.warranty, data.warranty, dt.warranty || ''));
      setWarrantyAr(pick(parsedDraft?.warrantyAr, data.warranty_ar, dt.warrantyAr || ''));
      setManpower(pick(parsedDraft?.manpower, data.manpower, dt.manpower || ''));
      setManpowerAr(pick(parsedDraft?.manpowerAr, data.manpower_ar, dt.manpowerAr || ''));
      setMobilization(pick(parsedDraft?.mobilization, data.mobilization, dt.mobilization || ''));
      setMobilizationAr(pick(parsedDraft?.mobilizationAr, data.mobilization_ar, dt.mobilizationAr || ''));
      setDuration(pick(parsedDraft?.duration, data.duration, dt.duration || ''));
      setDurationAr(pick(parsedDraft?.durationAr, data.duration_ar, dt.durationAr || ''));
      setBankDetails(pick(parsedDraft?.bankDetails, data.bank_details, dt.bankDetails || ''));
      setBankDetailsAr(pick(parsedDraft?.bankDetailsAr, data.bank_details_ar, dt.bankDetailsAr || ''));
      setFooter(pick(parsedDraft?.footer, data.footer, dt.footer || ''));
      setFooterAr(pick(parsedDraft?.footerAr, data.footer_ar, dt.footerAr || ''));
      ...
      setShowNote(pick(parsedDraft?.showNote, data.show_note !== undefined ? Boolean(data.show_note) : true, true));
      setShowPayment(pick(parsedDraft?.showPayment, data.show_payment !== undefined ? Boolean(data.show_payment) : true, true));
      setShowWarranty(pick(parsedDraft?.showWarranty, data.show_warranty !== undefined ? Boolean(data.show_warranty) : true, true));
      setShowManpower(pick(parsedDraft?.showManpower, data.show_manpower !== undefined ? Boolean(data.show_manpower) : true, true));
      setShowMobilization(pick(parsedDraft?.showMobilization, data.show_mobilization !== undefined ? Boolean(data.show_mobilization) : true, true));
      setShowDuration(pick(parsedDraft?.showDuration, data.show_duration !== undefined ? Boolean(data.show_duration) : true, true));
      setShowBankDetails(pick(parsedDraft?.showBankDetails, data.show_bank_details !== undefined ? Boolean(data.show_bank_details) : true, true));
      setShowCustomField(pick(parsedDraft?.showCustomField, data.show_custom_field !== undefined ? Boolean(data.show_custom_field) : (parsedCustomFields.length > 0), parsedCustomFields.length > 0));
  ...
  const clearForm = async () => {
    ...
    // Fetch and apply latest document terms from settings
    ...
    try {
      const dRes = await fetch('/api/settings/discountMode', { headers: getAuthHeaders() });
      if (dRes.ok) {
    ...
    if (product) {
      setItems(prevItems => {
        ...
        newItems[index] = {
          ...
          description_ar: translationMode === 'auto' ? (product.description_ar || '') : '',
          unit: product.unit,
          unit_price: newUnitPrice,
      ...
      // Only auto-translate if translationMode is 'auto'
  ...
  const toggleTranslationMode = async () => {
    const next = translationMode === 'manual' ? 'auto' : 'manual';
    ...
    try {
      await fetch('/api/settings', {
        ...
        body: JSON.stringify({ key: 'translationMode', value: next })
      });
    } catch {}
  };
  ...
  const saveDraftToDB = useCallback(async (isImmediate = false) => {
    ...
    const draft = {
      ...
      showNote, showPayment, showWarranty, showManpower, showMobilization, showDuration, showBankDetails, showCustomField,
  ...
  const handleServerPDF = async (includeStamp: boolean = false) => {
    ...
    try {
      ...
      params.set('showNote', String(showNote));
      params.set('showPayment', String(showPayment));
      params.set('showWarranty', String(showWarranty));
      params.set('showManpower', String(showManpower));
      params.set('showMobilization', String(showMobilization));
      params.set('showDuration', String(showDuration));
      ...
      const canvas = await html2canvas(printRef.current, {
        ...
        onclone: (clonedDoc) => {
          ...
          hideSelectors.forEach(sel => {
            try {
              clonedDoc.querySelectorAll(sel).forEach((el: any) => {
          ...
          clonedElements.forEach((el: any, index) => {
            if (el.closest('[data-html2canvas-ignore="true"]') || el.closest('[class*="print:hidden"]')) {
              el.style.display = 'none';
              return;
            }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              ...
              {saveStatus === 'unsaved' && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium text-gray-500 bg-gray-50 dark:bg-gray-800 dark:text-gray-400 rounded-full border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          ...
          <div className="flex items-center rounded-lg border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/20 overflow-hidden shadow-xs">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-indigo-700 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors font-medium text-sm"
              title="Translate all Arabic fields based on English content"
            >
            ...
            <button
              ...
              className="px-2.5 py-2 text-xs font-semibold text-indigo-600 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors flex items-center gap-1.5"
              title={`Translation mode: ${translationMode === 'manual' ? 'Manual (Only translates when clicking Translate)' : 'Auto (Translates automatically)'}. Click to toggle.`}
            >
              <span className={`w-2 h-2 rounded-full ${translationMode === 'manual' ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-lg">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      title={idUnlocked ? 'Lock ID' : 'Unlock to manually customize Quote ID'}
                    >
                      {idUnlocked ? <Unlock size={14} className="text-amber-600" /> : <Lock size={14} className="text-gray-400 hover:text-gray-600" />}
                    </button>
                  </div>
                      ...
                      >
                        ...
                        `}
                          ...
                          <div className="p-0 border-r border-gray-300 h-full flex relative group">
                            ...
                            <div className="px-2 py-1.5 w-1/2 flex flex-col justify-start relative group/ar">
              ...
              <div className="flex flex-col gap-y-0.5 mt-4">
                {showPayment ? (
                  <div className="flex flex-col md:flex-row md:items-center group">
                    ...
                    <input type="checkbox" data-html2canvas-ignore="true" className="print:hidden cursor-pointer h-4 w-4 shrink-0 mx-1" checked={showPayment} onChange={(e) => setShowPayment(e.target.checked)} title="Hide Payment" />
                  </div>
                ) : (
                  <div data-html2canvas-ignore="true" className="print:hidden flex items-center gap-2 text-gray-400 italic">
                    <input type="checkbox" data-html2canvas-ignore="true" className="cursor-pointer h-4 w-4" checked={showPayment} onChange={(e) => setShowPayment(e.target.checked)} title="Show Payment" />
                ...
                {showWarranty ? (
                  <div className="flex flex-col md:flex-row md:items-center group">
                    ...
                    <input type="checkbox" data-html2canvas-ignore="true" className="print:hidden cursor-pointer h-4 w-4 shrink-0 mx-1" checked={showWarranty} onChange={(e) => setShowWarranty(e.target.checked)} title="Hide Warranty" />
                  </div>
                ) : (
                  <div data-html2canvas-ignore="true" className="print:hidden flex items-center gap-2 text-gray-400 italic">
                    <input type="checkbox" data-html2canvas-ignore="true" className="cursor-pointer h-4 w-4" checked={showWarranty} onChange={(e) => setShowWarranty(e.target.checked)} title="Show Warranty" />
                ...
                {showManpower ? (
                  <div className="flex flex-col md:flex-row md:items-center group">
                    ...
                    <input type="checkbox" data-html2canvas-ignore="true" className="print:hidden cursor-pointer h-4 w-4 shrink-0 mx-1" checked={showManpower} onChange={(e) => setShowManpower(e.target.checked)} title="Hide Manpower" />
                  </div>
                ) : (
                  <div data-html2canvas-ignore="true" className="print:hidden flex items-center gap-2 text-gray-400 italic">
                    <input type="checkbox" data-html2canvas-ignore="true" className="cursor-pointer h-4 w-4" checked={showManpower} onChange={(e) => setShowManpower(e.target.checked)} title="Show Manpower" />
                ...
                {showMobilization ? (
                  <div className="flex flex-col md:flex-row md:items-center group">
                    ...
                    <input type="checkbox" data-html2canvas-ignore="true" className="print:hidden cursor-pointer h-4 w-4 shrink-0 mx-1" checked={showMobilization} onChange={(e) => setShowMobilization(e.target.checked)} title="Hide Mobilization" />
                  </div>
                ) : (
                  <div data-html2canvas-ignore="true" className="print:hidden flex items-center gap-2 text-gray-400 italic">
                    <input type="checkbox" data-html2canvas-ignore="true" className="cursor-pointer h-4 w-4" checked={showMobilization} onChange={(e) => setShowMobilization(e.target.checked)} title="Show Mobilization" />
                ...
                {showDuration ? (
                  <div className="flex flex-col md:flex-row md:items-center group">
                    ...
                    <input type="checkbox" data-html2canvas-ignore="true" className="print:hidden cursor-pointer h-4 w-4 shrink-0 mx-1" checked={showDuration} onChange={(e) => setShowDuration(e.target.checked)} title="Hide Duration" />
                  </div>
                ) : (
                  <div data-html2canvas-ignore="true" className="print:hidden flex items-center gap-2 text-gray-400 italic">
                    <input type="checkbox" data-html2canvas-ignore="true" className="cursor-pointer h-4 w-4" checked={showDuration} onChange={(e) => setShowDuration(e.target.checked)} title="Show Duration" />
                  ...
                  <div className="flex flex-col md:flex-row md:items-start group mt-1">
                    ...
                    <div className="flex-1 flex flex-col justify-center relative group/bank pr-2">
                      <textarea
                        ...
                        value={bankDetails}
                        onChange={e => { setBankDetails(e.target.value); autoResizeTextarea(e.currentTarget); }}
                        onBlur={() => handleAutoTranslate(bankDetails, bankDetailsAr, setBankDetailsAr)}
                        rows={Math.max(bankDetails.split('\n').length, 3)}
                      />
                    </div>
                    <textarea
                      data-autoresize="true"
                ...
                {showCustomField && customFields.length > 0 ? (
                  <div className="flex flex-col gap-2 mt-2">
                    {customFields.map((cf, index) => (
                      <div key={cf.id} className="flex flex-col md:flex-row gap-2 group relative">
                        ...
                        <div data-html2canvas-ignore="true" className="print:hidden flex items-center gap-1 mx-1 mt-1 shrink-0 absolute right-[-30px] top-0 md:static">
                    ...
                    <div data-html2canvas-ignore="true" className="print:hidden flex items-center mt-2 group relative h-4">
                      <input type="checkbox" data-html2canvas-ignore="true" className="absolute left-0 top-0 cursor-pointer h-4 w-4 z-10 opcaity-0 w-full h-full" checked={showCustomField} onChange={(e) => setShowCustomField(e.target.checked)} title="Hide Custom Fields" />
                  ...
                  <div data-html2canvas-ignore="true" className="print:hidden flex items-center gap-2 text-gray-400 italic mt-2">
                    <input type="checkbox" data-html2canvas-ignore="true" className="cursor-pointer h-4 w-4" checked={showCustomField} onChange={(e) => { if (e.target.checked && customFields.length === 0) addCustomField(); setShowCustomField(e.target.checked); }} title="Show Custom Field" />
                    <span>Show Custom Field</span>
                  </div>
                )}

                {showCustomField && (
                  <div data-html2canvas-ignore="true" className="print:hidden mt-2">
